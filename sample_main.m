...
for count=1:num
    ...
    %%  PSO PROGRAM
    max_iteration=500;
    simulations=5; % 30 for testing # of valid scheds case 1 vs case 2, 5 otherwise
    ...
    if (Monte==1);
       ...
       price_code=1;
        ...
        batoperation = batop(t, solution, app_TW, PV,MaxBattCharge,MinBattCharge, HighThresh, LowThresh);
    ...
    fitness(a,itectr)=sample_func(n, t, solution, price(price_code,:), app_usage, app_TW, app_dur, app_tA, app_tB, user_budget, peak_threshold(peak_code,:), mu, tot_prod,batoperation);
                end
    ...
    %%
            ...
            Singhouse_netLoad = zeros(1,t);
            Singhouse_Cost= zeros(1,t);
            Singhouse_Savings = 0;
            %Create Load per hour matrix for a single house
           
            
            
            for b=1:t
                for a=1:n
                    load_perApp_perHr(a,b) = solution(a,b)*app_TW(a);    %gets Wattage consumed per hr per appliance
                    %cost_perApp_perHr(a,b) = solution(a,b)*app_TW(a)*price(price_code,b);
                end
                  Singhouse_netLoad(b) = sum(load_perApp_perHr(:,b))-sum(PV(:,b)) + sum(batoperation(:,b)); %gets net wattage consumed per hr
                  Singhouse_Cost(b) = Singhouse_netLoad(b) * price(price_code,b);    %gets cost of consumed per hr
            end
            Singhouse_Savings = sum(Singhouse_Cost);
            
            %Load per hour for the community
            %done after every single house run for cumulative result
            
            for b=1:t
                AllHouse_netLoad(b) = AllHouse_netLoad(b) + Singhouse_netLoad(b);
                AllHouse_netCost(b) = AllHouse_netCost(b) + Singhouse_Cost(b);
            end
            
            %AllHouse_netLoad(b) is a (1,24) matrix that contains net consumption(Watts) per hour
            AllHouse_Savings = AllHouse_Savings + Singhouse_Savings;            
end        
